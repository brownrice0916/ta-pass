...
export default function RestaurantsPage() {
  ...
  const [selectedSocialMedia, setSelectedSocialMedia] = useState<Set<string>>(
    new Set()
  );
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          ...
          <DialogContent className="sm:max-w-[425px]">
            ...
            <form onSubmit={handleAddSelectedRestaurant} className="space-y-4">
              ...
              {/* {newRestaurant.name && (
                <div className="space-y-2">
                  ...
                  <div className="text-sm">
                    <p>
                      <strong>이름:</strong> {newRestaurant.name}
                    </p>
                    <p>
                      <strong>주소:</strong> {newRestaurant.address}
                    </p>
                    <p>
                      <strong>카테고리:</strong>{" "}
                      {CATEGORIES.find(
                        (cat) => cat.value === newRestaurant.category
                      )?.label ||
                        newRestaurant.category ||
                        "정보 없음"}
                    </p>
                    <p>
                      <strong>위도:</strong> {newRestaurant.latitude.toFixed(6)}
                    </p>
                    <p>
                      <strong>경도:</strong>{" "}
                      {newRestaurant.longitude.toFixed(6)}
                    </p>
                  </div>
                </div>
              )} */}
              {/* 체크박스 그룹 */}
              <p className="text-sm font-semibold">연동할 소셜미디어를 선택 </p>
              <div className="flex flex-wrap gap-2 mb-4">
                {SOCIAL_MEDIA_OPTIONS.map(({ key, label }) => (
                  <div key={key} className="flex items-center space-x-2">
                    <input
                      type="checkbox"
                      id={`social-${key}`}
                      checked={selectedSocialMedia.has(key)}
                      onChange={(e) => {
                        const newSelected = new Set(selectedSocialMedia);
                        if (e.target.checked) {
                          newSelected.add(key);
                        } else {
              ...
              {/* 선택된 소셜 미디어에 대한 입력 필드만 표시 */}
              <div className="space-y-4">
                {SOCIAL_MEDIA_OPTIONS.filter(({ key }) =>
                  selectedSocialMedia.has(key)
            ...
            <div className="p-4 space-y-2">
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">
                  {CATEGORIES.find((cat) => cat.value === restaurant.category)
                    ?.label || "기타"}
                </p>
              </div>
